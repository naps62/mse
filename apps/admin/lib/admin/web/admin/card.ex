defmodule Admin.Web.ExAdmin.Card do
  ...
  register_resource DB.Models.Card do
    ...
    scope(
      :no_data_at_all,
      &where(&1, [c], is_nil(c.gatherer_data) and is_nil(c.mtgjson_data))
    )
    ...
    index do
      ...
      column(
        :mkm_detailed_updated_at,
        &Helpers.relative_date(&1.mkm_detailed_updated_at)
      )
    end
    ...
    show card do
      ...
      attributes_table "Magic Card Market" do
        ...
        row(
          :mkm_basic_updated_at,
          &Helpers.relative_date(&1.mkm_basic_updated_at)
        )
