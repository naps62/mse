defmodule Mtgio.Sets do
  ...
  def add_set(set_data, multi) do
    case find_set(set_data) do
      nil ->
        IO.puts "Could not find matching set for #{set_data[:name]}"
        multi

      set ->
        Multi.update(multi, set_data.code, set_changeset(set, set_data))
    end
  end

  def find_set(%{name: name}) do
    Set |> where(name: ^name) |> SilentRepo.one
  end
